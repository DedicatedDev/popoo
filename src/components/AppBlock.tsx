...
//@ts-ignore: Duplicated define in library.
  ...
  return (
    <>
      ...
      <Animated.View style={[styles.container, containerZIndex]}>
        <Animated.View style={[boxWrapperStyles, { position: "absolute" }]} ref={boxWrapperRef}>
          <AnimatedScrollView
            ...
            onScroll={useAnimatedScrollHandler((event) => {
              if (post.textStyle === "dark")
                closeBtnTint.value =
                  event.contentOffset.y > THUMBNAIL_HEIGHT * THUMBNAIL_IMAGE_SCALE_FACTOR - 35 ? "light" : "dark";
              const scrollViewHeight = measure(scrollViewRef).height;
              scrollViewBounces.value = event.contentOffset.y > scrollViewHeight / 2;
            })}
            scrollEventThrottle={16}
          >
            <AnimatedPanGestureHandler onGestureEvent={gestureEventHandler} animatedProps={panGestureHandlerProps}>
              <Pressable
                onPress={() => {
              ...
              >
                <Animated.View style={thumbnailStyles}>
                  <View style={styles.titleWrapper}>
                    <Text style={[styles.title, { color: post.textStyle === "dark" ? "#303030" : "#FFF" }]}>
                      {post.title}
                    </Text>
                  </View>
                  <Animated.View
                    style={
                      Platform.OS === "android"
                        ? [Android_thumbnailImageStyles, { overflow: "hidden", flex: 1 }]
                        : { overflow: "hidden", flex: 1 }
                    }
                  >
                    <Image source={{ uri: post.imageUrl }} style={styles.imageStyles} resizeMode="cover" />
                  </Animated.View>
                  <View style={styles.appDetailsWrapper}>
                    ...
                    >
                      <Image style={[styles.avatar, { marginRight: 16 }]} source={{ uri: post.ownerAvatarUrl }} />
                      <View>
                        <Text style={[styles.subtitle, { color: post.textStyle === "dark" ? "#4B4B4B" : "#FFF" }]}>
                          {post.subtitle}
                        </Text>
                        <Text
                          numberOfLines={2}
                          style={[styles.description, { color: post.textStyle === "dark" ? "#4B4B4B" : "#FFF" }]}
                        >
                          {post.description}
                        </Text>
                      </View>
                    ...
                    <View>
                      <TouchableHighlight style={styles.get} onPress={() => {}} underlayColor="#fff">
                        <Text style={[styles.getText]}>GET</Text>
                      </TouchableHighlight>
                      <Text style={[styles.inAppText, { marginTop: 4 }]}>In-App Purchase</Text>
                    </View>
                  </View>
                </Animated.View>
              </Pressable>
          ...
          {Platform.OS !== "android" && (
            <CloseButton opacity={closeBtnOpacity} hideAppDetails={hideAppDetails} closeBtnTint={closeBtnTint} />
          )}
        </Animated.View>
      </Animated.View>
    </>
